...
3. **Given** the `Shortener/` and `Statistics/` bounded contexts, **When** a maintainer inspects runtime folders, **Then** separate folders exist for Shortener API, Redirect Service, Statistics.Api, Statistics.EventWriter.Worker, Statistics.BatchProcessor.Worker, and Statistics.Infrastructure.
...
1. **Given** the repository foundation is present, **When** a developer opens `DistributedUrlShortener.sln`, **Then** it includes the Shortener API, Redirect Service, Statistics.Api, Statistics.EventWriter.Worker, Statistics.BatchProcessor.Worker, and Statistics.Infrastructure projects.
...
2. **Given** the repository foundation, **When** a developer opens `docker-compose.yml`, **Then** it uses environment variables where practical and remains intentionally minimal for Phase 0.
3. **Given** later infrastructure work begins, **When** Phase 1 extends local services, **Then** the Phase 0 Compose and environment baseline can be extended without changing bounded-context ownership.
...
2. **Given** a reviewer opens `docs/architecture.md`, **When** they read the request-flow overview, **Then** it clearly states that public `GET /{shortCode}` is served by Redirect Service outside ApiGateway and authenticated APIs go through ApiGateway.
...
- API health endpoints must be limited to shell readiness and must not imply dependency readiness for services not introduced in Phase 0.
- Local configuration examples must avoid real credentials, tokens, private endpoints, or production-like secrets.
- Compose content must not accidentally introduce Phase 1 infrastructure scope such as Cassandra schema, Redis behavior, Redpanda topics, MinIO buckets, ClickHouse tables, Keycloak realm setup, or observability services.
...
- **FR-004**: The Statistics bounded context MUST contain separate project folders for Statistics.Api, Statistics.EventWriter.Worker, Statistics.BatchProcessor.Worker, and Statistics.Infrastructure.
...
- **FR-007**: The solution MUST include real project shells for Shortener API, Redirect Service, Statistics.Api, Statistics.EventWriter.Worker, Statistics.BatchProcessor.Worker, and Statistics.Infrastructure.
...
- **FR-014**: The Phase 0 scope MUST NOT implement URL creation, redirect resolution, Cassandra schema, Redis cache behavior, Redpanda event publishing, MinIO/Parquet writing, DuckDB batch processing, ClickHouse dashboard queries, Keycloak integration, Angular frontend implementation, observability stack, or load tests.
- **FR-015**: `docker-compose.yml` MUST provide a minimal local-development foundation that is easy to extend in Phase 1.
- **FR-016**: `docker-compose.yml` MUST use variables from `.env.example` where practical and MUST NOT contain committed secrets.
- **FR-017**: `.env.example` MUST document required Phase 0 variables with safe example values and without real secrets.
- **FR-018**: `docs/architecture.md` MUST describe the intended high-level architecture and the main bounded contexts: ApiGateway, User, Shortener, Statistics, and Front.
- **FR-019**: `docs/architecture.md` MUST explicitly document that public `GET /{shortCode}` is served by Redirect Service outside ApiGateway.
- **FR-020**: `docs/architecture.md` MUST document that authenticated APIs go through ApiGateway.
- **FR-021**: `docs/architecture.md` MUST include the planned analytics flow at a high level without implementing analytics behavior.
- **FR-022**: `docs/tradeoffs.md` MUST document initial architectural trade-offs, including separate Redirect Service, separate Statistics runtimes, project shells before business behavior, and known Phase 0 limitations.
- **FR-023**: All generated project documentation, specs, plans, task lists, architecture docs, and project-doc comments MUST be written in English.
- **FR-024**: Basic build verification MUST be documented as a first-class requirement for Phase 0 completion.
- **FR-025**: Health checks, configuration, documentation, and build verification MUST be treated as first-class Phase 0 requirements rather than follow-up cleanup.

### Constitution Alignment *(mandatory)*

- **Redirect Path Impact**: Phase 0 creates only a separate Redirect Service shell. It must preserve the future public `GET /{shortCode}` boundary outside ApiGateway and must not implement redirect logic.
...
- **Analytics Impact**: Phase 0 creates only Statistics runtime shells and high-level analytics documentation. It must not implement ClickEvent publishing, broker consumption, Parquet storage, batch processing, scheduling, ClickHouse writes, or dashboard queries.
...
- **SC-008**: `.env.example` and `docker-compose.yml` contain 0 committed real secrets and provide a baseline that can be extended in Phase 1.
